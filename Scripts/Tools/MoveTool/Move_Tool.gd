...
func event_1(event):
	...
	if event.is_action("Click") and Input.is_action_just_released("Click"):
		#if mouse was still commit SELECT command
		if (Cpos == Cstart_pos):
			...
			if !Input.is_action_pressed("Shift"):
				...
				if !result and TH.do_undo.get_cur_selected().size() == 0:
					return
			#commiting command
			command_sel.add_to_group("CurSelected_nodes")
			command_sel.DoUndo = TH.do_undo
			TH.do_undo.add_command(command_sel,"select_")
			$Directions.upd_vis()										#DIR UPD
		#if we dragged somewhere, commit MOVE command
	...
	if Input.is_action_pressed("Click"):
			...
			if  TH.Map.if_select_exists():
				move()
			$Directions.upd_mov(TH.Map.get_select_position())		#DIR UPD
