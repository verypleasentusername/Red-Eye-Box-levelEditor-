...
#			print("start points %s" % points)
			...
			var normal:Vector3 = Math_U.face_area_x2(points).normalized()
#			print("normal %s" % normal)
			triangulation_indices = Math_U.trianglate_face_vertex_indices(points, normal)
...
func init_prism(base_points:Array[Vector3], extrude_dir:Vector3, uv_transform:Transform2D = Transform2D.IDENTITY, material_id:int = -1, visible:bool = true, color:Color = Color.WHITE):
	...
	var face_area_x2:Vector3 = Math_U.face_area_x2(base_points)
...
#			var v_idx:int = data.face_vertex_indices[count]
		...
		var normal = Math_U.face_area_x2(vert_points).normalized()
	...
	for v_idx in vertices.size():
		...
		for face in faces:
			if face.vertex_indices.has(v_idx):
				weighted_normal += Math_U.face_area_x2(face.get_points())
...
#
	...
	for f in faces:
		...
		planes.append(Math_U.flip_plane(f.get_plane()))
	planes.append(plane)
	
	#print("planes %s" % GeneralUtil.format_planes_string(planes))
	
	var hull_points:Array[Vector3] = Math_U.get_convex_hull_points_from_planes(planes)
	...
	for f in new_vol.faces:
		var f_plane:Plane = Math_U.flip_plane(f.get_plane())
...
func translate_face_plane(face_id:int, offset:Vector3, lock_uvs:bool = false)->ConvexVolume:
	...
	for f in faces:
		if f.id == face_id:
			transformed_plane = Math_U.flip_plane(f.get_plane()) * xform
...
#	var xform:Transform3D = obj_xform.affine_inverse()
	...
	for f in faces:
		var axis:Math_U.Axis = Math_U.get_longest_axis(f.normal)
...
#			print("f_idx %s" % f_idx)
			...
			var axis:Math_U.Axis = Math_U.get_longest_axis(face.normal)
	...
	#			var p:Vector3 = triangles[i]
				...
				var uv:Vector2
				if axis == Math_U.Axis.X:
					uv = Vector2(-p.z, -p.y)
				elif axis == Math_U.Axis.Y:
					uv = Vector2(-p.x, -p.z)
				elif axis == Math_U.Axis.Z:
					uv = Vector2(-p.x, -p.y)
...
func append_mesh_outline(mesh:ImmediateMesh, viewport_camera:Camera3D, local_to_world:Transform3D, material:Material, thickness:float = 4):
	...
	for edge in edges:
		...
		for p in frustum:
			var p_flip:Plane = Math_U.flip_plane(p)
			var result:PackedVector3Array = Math_U.clip_segment_to_plane_3d(p_flip, p0_world, p1_world)
			if result.is_empty():
				frustum_culled = true
				break
			p0_world = result[0]
	...
	var loops:Array[Loop2D] = Math_U.get_loops_from_segments_2d(segments)
	for loop in loops:
...
#		var tris:PackedVector3Array = MathUtil.trianglate_face(face.get_points(), face.normal)
		...
		for i in range(0, tris.size(), 3):
			...
			var p_hit:Vector3 = Math_U.intersect_plane(origin, dir, p0, tri_area_x2)
			if !p_hit.is_finite():
				continue
...
func subtract(subtrahend:ConvexVolume)->Array[ConvexVolume]:
	...
	for face in subtrahend.faces:
		...
		var vol_under:ConvexVolume = split_vol.cut_with_plane(Math_U.flip_plane(p))
...
func intersects_frustum(frustum:Array[Plane])->bool:
	...
	for face in faces:
		...
		if Math_U.polygon_intersects_frustum(points, frustum):
			return true
